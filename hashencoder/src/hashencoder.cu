...
// B, D  batch size, coord dim
// L level
// C embedding dim for each level
// H base resolution
...
__global__ void kernel_grid(
    ...
    // locate
    grid += (uint32_t)offsets[level] * C; //定位某个级别的哈希网格对应哪一块数据
    inputs += b * D;
    ...
    // 这里算完后 pos_grid存了采样点对应的网格3顶点, pos 存了采样点在网格内的小数偏移，用于后续插值
