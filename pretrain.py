...
from dataset_pretrain import TrainData
from tqdm import tqdm
...
# import config
config_path = "config_pretrain.json"
...
# init dataset
train_dataset = TrainData(input, num_sample_point)
...
writer = tensorboard.SummaryWriter('Pretrain_log/{}'.format(name))

for i in loop_tqdm:
        NeRF.train()
        epoch_loss = 0
 
        for index, (ray,projection) in enumerate(TrainLoader):
                
                projection = projection.reshape(-1).float().cuda()
                ray = ray.reshape(-1,3).float().cuda()
                u = NeRF(ray).reshape(-1)

                loss = lossfn(u,projection)
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
        ...
        with torch.no_grad():
                if (i+1) % config['train']['save_epoch'] == 0:
                    ...
                    img_jpg_w = NeRF(slice_w).reshape(h,l).float().cpu().detach().numpy()
                    img_jpg_h = NeRF(slice_h).reshape(w,l).float().cpu().detach().numpy()
                    img_jpg_l = NeRF(slice_l).reshape(w,h).float().cpu().detach().numpy()

                    img_jpg = img_jpg_w

                    data = img_jpg.reshape(-1)
                ...
                if (i+1) % (config['train']['save_epoch']*5) == 0:
                        NeRF = NeRF.cpu()
                        ...
                        for slice_index in range(w):
                                ...
                                slice_pre = NeRF(slice).reshape(1,h,l)
                                if slice_index==0:
                                        img_pre = slice_pre
                                else:
